...
for candidate in "$script_root_guess" "$submit_root"; do
	if [[ -f "$candidate/renv/activate.R" && -f "$candidate/simulations/missing_outcomes/miss_scripts/run_missing_tmle.R" ]]; then
...
if [[ -z "$project_root" ]]; then
	echo "Unable to locate project root with renv/activate.R and run_missing_tmle.R" >&2
