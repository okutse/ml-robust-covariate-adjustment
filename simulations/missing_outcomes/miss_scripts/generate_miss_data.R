# This script generates datasets with missing outcomes under different settings of missingness mechanisms and varying R^2 values for the outcome model achieved by varying the noise levels in the data generating model. The generated datasets are saved in .RData files for downstream analysis.

# Outputs are saved in a structured directory format under "simulations/missing_outcomes/miss_datasets", with sub-folders for each missingness setting and files named according to the sample size and target R^2 value.
...
generate_missing_data <- function(n, target_r2, max_attempts = 50, eta0, eta1, setting) {
...
base_seed <- 12345
settings <- list(
  setting_one = list(eta0 = 0, eta1 = 0),
  setting_two = list(eta0 = 1, eta1 = 0),
  setting_three = list(eta0 = 1, eta1 = 1)
)
