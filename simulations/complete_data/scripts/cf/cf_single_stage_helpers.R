# Cross-fitted single-stage helpers (K=the number of folds and is configurable below)
...
write_replicate_cache <- function(method_dir, rep_row) {
  ...
  # Update a combined checkpoint file for quick resume checks. Consolidate
  # all per-replicate checkpoint files and write the combined view.
...
consolidate_replicate_checkpoints <- function(method_dir) {
  ...
  # Read any consolidated checkpoint file if present, then per-replicate checkpoint files.
  combined_list <- list()
  combined_file <- file.path(method_dir, "replicate_checkpoint.csv")
...
crossfit_single_stage <- function(df, fit_fun, pred_fun, v = CF_FOLDS) {
  ...
  for (split in folds$splits) {
    ...
    mu0[idx] <- pred_fun(fit_obj, assess_A0)
    mu1[idx] <- pred_fun(fit_obj, assess_A1)
  }
...
get_single_stage_cf_registry <- function() {
  list(
...
extract_single_stage_metrics_cf <- function(res) {
...
summarize_single_stage_cf_metrics <- function(metrics) {
  ...
  run_method <- function(method) {
    ...
    metrics <- foreach::foreach(
      ...
      estimate_from_df <- function(dat) {
        ...
        point_m <- extract_single_stage_metrics_cf(point_res)
    ...
    summary_res <- summarize_single_stage_cf_metrics(metrics)
