...
method_registry <- get_single_stage_cf_registry()
...
scenario_files <- setdiff(scenario_files, file.path(output_dir, "single_stage_cf_results.csv"))
if (length(scenario_files) > 0) {
  all_results <- lapply(scenario_files, read.csv, stringsAsFactors = FALSE) %>% dplyr::bind_rows()
  write.csv(all_results, file.path(output_dir, "single_stage_cf_results.csv"), row.names = FALSE)
  cat("Wrote aggregate:", file.path(output_dir, "single_stage_cf_results.csv"), "\n")
}
