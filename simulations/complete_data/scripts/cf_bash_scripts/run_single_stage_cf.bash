#!/bin/bash
#SBATCH --job-name=complete_data_single_stage_cf
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
for candidate in "$script_root_guess" "$submit_root"; do
	if [[ -f "$candidate/renv/activate.R" && -f "$candidate/simulations/complete_data/scripts/cf/single_stage_cf_results.R" ]]; then
...
if [[ -z "$project_root" ]]; then
	echo "Unable to locate project root with renv/activate.R and single_stage_cf_results.R" >&2
